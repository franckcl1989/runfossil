...
| RK-013 | Service, log, or journal support regresses to command wrappers. | medium | controlled | Bounded local log file windows and native service protocols are used where supported; `journalctl` and other command wrappers remain prohibited. | Coverage Decision Matrix, Safety and Operations |
| RK-014 | Hardware management requires vendor CLIs or C SDKs. | medium | controlled | Vendor-specific hardware management is deferred or excluded; Linux-exposed sysfs sensor, thermal, and hwmon state is captured. | Coverage Decision Matrix |
| RK-015 | Container-related evidence is mistaken for daemon API coverage. | medium | controlled | Capture host-side procfs, namespace, cgroup, and runtime socket metadata evidence; container daemon APIs and image contents are excluded from core scope. | Source Taxonomy, Coverage Decision Matrix |
...
| RK-040 | Completeness expectations drive unbounded capture or scope creep. | high | controlled | Define completeness as accountable Linux runtime evidence within scope and budget; planner records omissions instead of overrunning configured limits or expanding into application, cloud, database, language-runtime, or orchestration data. | Project Design, Capture Planning Strategy, Requirements Traceability |
